...
/** Integer paise. */
export const zMoney = z.number().int().min(0, "Amount cannot be negative").max(2_147_483_647, "Amount is too large");
...
export const zQty = z.coerce.number().int().min(1, "Quantity must be at least 1").max(100000, "Quantity is too large");
export const zId = z.coerce.number().int().positive().max(2_147_483_647);
export const zVersion = z.coerce.number().int().min(1).max(2_147_483_647);
